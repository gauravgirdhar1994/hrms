...
class AttendanceUpload extends Component {
    ...
    refreshEvent(){
      var url = config.API_URL+"/upload-sheet-list";
    ...
    render() {   
      ...
      <tbody>
       ...
       <td>{moment(obj.createdOn).format(config.DATE_FORMAT)}</td>
          <td><a href={(obj.is_processed)?config.BASE_URL+'/'+obj.uploadedFile:config.BASE_URL+'/'+obj.errorFile} class="btn btn-sm btn-outline-danger ml-2 " download>Download</a></td>
